...
class DatabaseHelper {
  ...
  final String tableName = "catodoTbl";
  ...
  initDb() async {
    ...
    String path = join(documentDirectory.path, "catodo_db.db");
...
//insertion
  Future<int> saveItem(CaToDoItem item) async {
...
//
  Future<CaToDoItem> getItem(int id) async {
    var dbClient = await db;
    var result = await dbClient.rawQuery("SELECT * FROM $tableName WHERE id = $id");
    if (result.length == 0) return null;
    return new CaToDoItem.fromMap(result.first);
  }
...
//  }
  ...
  Future<int> updateItem(CaToDoItem item) async {
    var dbClient = await db;
